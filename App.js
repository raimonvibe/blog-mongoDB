...
app.get("/", function(req, res){
  if (process.env.DATABASE_URL && process.env.DATABASE_URL.trim() !== '' && process.env.DATABASE_URL.startsWith('mongodb')) {
    ...
    const demoPost = {
      _id: "demo1",
      title: "Welcome to RaimonVibe Blog",
      content: "This is a demo post showing the blog functionality. When you connect your MongoDB database, you'll be able to create and manage real posts."
    };
    res.render("home", {
      startingContent: homeStartingContent,
      posts: [demoPost]
...
app.post("/edit/:postId", function(req, res){
  ...
  Post.findByIdAndUpdate(requestedPostId, {
    title: req.body.postTitle,
    content: req.body.postBody
  }, function(err){
    if (!err){
